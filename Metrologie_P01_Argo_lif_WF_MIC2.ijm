...
// Known problems:
//	Detection of squares by Auto-Threshold may fail if image is blurry
...
var dotsWidth = 51.5;//dimension mesuree

/* Dimensions de la mosaique Argolight de 4x4 carres en microns
 * de centre a centre des carres extremes 
 * (voir image  2021_04_05_Métrologie_MIC2.lif - P01_40x_gfp_900ms_100_MesurePasDuReseau)
 * TopLeft - TopRight : 178.7 micron 
 * BottomLeft - BottomRight :  178.0 
 * TopLeft - BottomLeft  : 178.8 
 * TopRight - BottomRight : 179.1 
 * Moyenne : 178.65 microns */
var moyenne4mesures = (178.7 + 178.0 + 178.8 + 179.1)/4;// = 178.65 microns;
var dotsArrayWdth = moyenne4mesures;
var dotsDetectionFailed;
...
var elementarySquareArea = 2810; //moyenne aire des 16 carres segmentes par Triangle
...
var gridWidth = dotsArrayWdth;
var gridHeight = dotsArrayWdth;
var roisWidth = dotsWidth * 0.9;//physical units, for grid computation
...
var brightestSquarePosition = "TopLeft";//"TopRight" pour MIC1
var scanDirection;
...
function getParams() {
	Dialog.create("Metrologie_MIC2");
	scanDirections = newArray("LeftToRight", "RightToLeft");
...
function getDotsAsRois(id) {
	...
	tolerance = 20;// %
	tolerance = 10;// < 10 => problemes de detection
	...
	setAutoThreshold("Triangle dark");
	if (toLowerCase(objective) == "20x") {
		//Draw a square in center of image to exclude other paterns
		run("Specify...", "width=1124 height=1124 x=461 y=461");//MIC2
		//MIC1
		//print("height = "+height);
		//run("Specify...", "width="+(3*width/5)+" height="+(3*height/4)+" x="+(1*width/5)+" y="+(1*height/8));
	}
	//exit;
//	wait(1000);
...
function createRoiGrid(centerX, centerY, cols, rows,
	...
	xs = xStep; ys = yStep;
	roisWidthPixels = roisWidth / sqrt(pixelWidth*pixelHeight);
